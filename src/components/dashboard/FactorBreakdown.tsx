...
import { Waves, CloudRain, Mountain, History, CheckCircle, Calculator } from 'lucide-react';
...
export const FactorBreakdown: React.FC<FactorBreakdownProps> = ({
  ...
  const factors = [
    {
      ...
      sourceBadgeColor: 'bg-amber-500/10 text-amber-300 border-amber-500/30',
    ...
    },
    {
      ...
      sourceBadgeColor: 'bg-purple-500/10 text-purple-300 border-purple-500/30',
    ...
    },
    {
      ...
      sourceBadgeColor: 'bg-pink-500/10 text-pink-300 border-pink-500/30',
  ...
  return (
    <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 shadow-lg space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <h3 className="text-base font-bold text-white flex items-center gap-2">
            ...
            <span className="text-xs font-normal px-2 py-0.5 rounded bg-slate-800 text-cyan-400 border border-slate-700">
              MCDA Multi-Criteria Engine
            </span>
          </h3>
          <p className="text-xs text-slate-400 mt-0.5">
            Transparent breakdown of component subscores, weights, data sources, and final point contributions
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {factors.map((f) => {
          ...
          return (
            <div
              key={f.id}
              className="bg-slate-950/60 rounded-xl p-4 border border-slate-800/80 hover:border-slate-700/80 transition-all flex flex-col justify-between"
            >
              <div>
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className={`text-[9px] font-semibold px-1.5 py-0.2 rounded border ${f.sourceBadgeColor}`}>
                          {f.sourceType}
                        </span>
                      </div>
                      <span className="text-[10px] text-slate-400 font-medium">
                        Model Weight: <strong className="text-slate-200">{f.weight}</strong>
                      </span>
                    </div>
                  </div>

                  <div className="text-right shrink-0">
                    <div className="text-base font-black tracking-tight" style={{ color: scoreColor }}>
      ...
      <div className="bg-slate-950 rounded-xl p-3.5 border border-cyan-500/30 text-xs flex flex-wrap items-center justify-between gap-3">
        ...
        <span className="px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 text-[10px] font-bold tracking-wide uppercase">
          100% Mathematically Consistent
        </span>
      </div>
    </div>
  );
